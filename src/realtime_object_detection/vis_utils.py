...
def visualize_boxes_and_labels_on_image_with_lidar(
    image,
    boxes,
    classes,
    scores,
            ...
            if scores is None or scores[i] > min_score_thresh:
                ...
                if not display_str:
                    ...
                    if scores is None:
                        ...
                        display_str = '{}: {}%'.format(display_str, int(100*scores[i])) 
                if distances[j] == 'None' or angles[j] == 'None':
                    dist_str = 'None'
                    angle_str = 'None'
                else:
                    dist_str = str(round(distances[j], 2))
                    angle_str = str(round(angles[j]))
                display_str = display_str + ' DIS: ' + dist_str + 'M DEG: ' + angle_str
                box_to_display_str_map[box].append(display_str)
                box_to_color_map[box] = STANDARD_COLORS[classes[i] % len(STANDARD_COLORS)]
            j = j + 1
    first = True
...
def draw_text_on_image(image, string, position, color = (0, 0, 255)):
    """
...
def visualize_objectdetection(image,
    ...
    if visualize:
        if lidar_use:
            visualize_boxes_and_labels_on_image_with_lidar(
            image,
            boxes,
            classes,
            scores,
